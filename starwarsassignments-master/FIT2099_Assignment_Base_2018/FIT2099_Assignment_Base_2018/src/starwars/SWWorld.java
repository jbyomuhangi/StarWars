...
public class SWWorld extends World {
	 ...
	 * and the grid.
	 */
	public SWWorld(int dimention) {
		SWLocation.SWLocationMaker factory = SWLocation.getMaker();
		myGrid = new SWGrid(factory, 10);
		space = myGrid;
	...
	public void initializeWorld(MessageRenderer iface) {
		...
		// A Droid 
		Droid droid1 = new Droid(Team.GOOD,50,iface,this,luke);
		droid1.setSymbol("QD");
		droid1.setShortDescription("Luke's Droid");
		loc = myGrid.getLocationByCoordinates(5,9);
		entityManager.setLocation(droid1, loc);
		
		// A Droid 
		Droid droid2 = new Droid(Team.EVIL,50,iface,this,null);
		droid2.setSymbol("QD");
		droid2.setShortDescription("Random droid");
		loc = myGrid.getLocationByCoordinates(2,2);
		entityManager.setLocation(droid2, loc);
		
		// A sandcrawler
		Sandcrawler sand = new Sandcrawler(Team.GOOD, 1000, iface, this, patrolmoves);
		sand.setSymbol("☺");
		loc = myGrid.getLocationByCoordinates(4,5);
		...
		//A grenade
		Grenade a = new Grenade(iface, this);
		a.setSymbol("G");
		loc = myGrid.getLocationByCoordinates(5, 9);
		entityManager.setLocation(a, loc);

		//A grenade
		Grenade b = new Grenade(iface, this);
		a.setSymbol("G");
		loc = myGrid.getLocationByCoordinates(6, 9);
		entityManager.setLocation(b, loc);

		//A grenade
		Grenade c = new Grenade(iface, this);
		a.setSymbol("G");
		loc = myGrid.getLocationByCoordinates(7, 9);
		entityManager.setLocation(c, loc);

		//A grenade
		Grenade d = new Grenade(iface, this);
		a.setSymbol("G");
		loc = myGrid.getLocationByCoordinates(8, 9);
	...
	public SWGrid getMyGrid(){return (SWGrid)space;}
