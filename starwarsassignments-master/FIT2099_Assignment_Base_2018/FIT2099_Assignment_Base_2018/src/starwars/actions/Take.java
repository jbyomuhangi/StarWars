...
public class Take extends SWAffordance {
	...
	public void act(SWActor a) {
		if (target instanceof SWEntityInterface) {
			...
			target.addAffordance(new Leave(theItem,messageRenderer)); // add leave affordance
			
			if (target instanceof Grenade) {
				target.addAffordance(new ThrowGrenade((SWEntityInterface) target,messageRenderer,((Grenade) target).getWorld()));
				((Grenade) target).setOwner(a);
			}
		}
	}
