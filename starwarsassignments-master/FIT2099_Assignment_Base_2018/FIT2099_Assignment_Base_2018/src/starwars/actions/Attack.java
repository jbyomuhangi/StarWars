...
public class Attack extends SWAffordance implements SWActionInterface {
	...
	/**
	 * An auxillary method for the method <code>act</code> that performs the <code>Attack</code> command on an entity.
	 * <p>
	 * This method deals damage(attacks) to the target according to the hitpoints of the item carried.
	 * It then deducts hitpoints from the item carried and finally deducts energy from the actor.
	 *
	 * @author 	omar -  refactored code from the method <code>act</code>
	 * @param 	a the <code>SWActor</code> who is attacking
	 * @param target the <code>SWActor</code> who is being attacked
	 * @param itemCarried the item being carried by the actor <code>a</code>
	 * @param energyForAttackWithWeapon the amount of energy required to attack with a weapon
	 ...
	 * @see 	starwars.Team
	 */
	private void act_aux(SWActor a, SWEntityInterface target, SWEntityInterface itemCarried, int energyForAttackWithWeapon){
		...
		if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {//if the actor is carrying an item 
				if (itemCarried.hasCapability(Capability.WEAPON)) {
                    if (itemCarried.getShortDescription() == "A Lightsaber"){
                        if (a.getForce() >= 60) {
                            act_aux(a,target, itemCarried, energyForAttackWithWeapon);}
                        else {a.say(a.getShortDescription()+"'s force level is too low to use a lightsabre."); }}
                    else {
						act_aux(a,target, itemCarried, energyForAttackWithWeapon);
                    }
				}
			...
			if (this.getTarget().getHitpoints() <= 0) {  // can't use isDead(), as we don't know that the target is an actor
				if(target instanceof Droid != true) {
					target.setLongDescription(target.getLongDescription() + ", that was killed in a fight");
				}
				else {
					target.setLongDescription(target.getLongDescription() + ", that was incapacitated in a fight");
				}
