...
public abstract class SWActor extends Actor<SWActionInterface> implements SWEntityInterface {
	...
	/** The amount of </code>forceAbility</code> this actor has */
	private int forceAbility = 0;
	...
	public SWActor(Team team, int hitpoints, MessageRenderer m, World world) {
		super(m);
	...
	public void resetMoveCommands(Location loc) {
		...
		for (CompassBearing d: CompassBearing.values()) { 														  
			if (loc.getNeighbour(d) != null) //if there is an exit from the current location in direction d, add that as a Move command
				newActions.add(new Move(d,messageRenderer, (SWWorld) world)); 
		}
	...
	/**
	 * This method will set a <code>SWActor</code>'s force level.
	 * <p> Actors will start off with a force level of 0 <p>
	 * @param force
	 */
	...
	/**
	 * This method will return an integer representing an Actor's force level.
	 * @return forceAbility 
	 */
	public int getForce() {
		return forceAbility;
